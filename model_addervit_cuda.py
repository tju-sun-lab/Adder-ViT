...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        """Vanilla ViT"""
        xq = self.queries(x).reshape(batch_size, seq_length, self.n_attention_heads, self.head_embed_dim)   # B, K, E -> B, K, H, HE        [400, 50, 128] -> [400, 50, 4, 32]
        xq = xq.transpose(1, 2)                                                                             # B, K, H, HE -> B, H, K, HE    [400, 50, 4, 32] -> [400, 4, 50, 32]

        xk = self.keys(x).reshape(batch_size, seq_length, self.n_attention_heads, self.head_embed_dim)      # B, K, E -> B, K, H, HE        [400, 50, 128] -> [400, 50, 4, 32]
        xk = xk.transpose(1, 2)                                                                             # B, K, H, HE -> B, H, K, HE    [400, 50, 4, 32] -> [400, 4, 50, 32]

        xv = self.values(x).reshape(batch_size, seq_length, self.n_attention_heads, self.head_embed_dim)    # B, V, E -> B, V, H, HE        [400, 50, 128] -> [400, 50, 4, 32]
        xv = xv.transpose(1, 2)                                                                             # B, V, H, HE -> B, H, V, HE    [400, 50, 4, 32] -> [400, 4, 50, 32]
...
class Encoder(nn.Module):
    def __init__(self, args):
        ...
        """Adder-ViT-cuda modification"""
        self.fc1 = UnoptimizedLinear(args.embed_dim, args.embed_dim * args.forward_mul)   # [400, 50, 128] -> [400, 50, 256]
        self.fc2 = UnoptimizedLinear(args.embed_dim * args.forward_mul, args.embed_dim)   # [400, 50, 256] -> [400, 50, 128]
